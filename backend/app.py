...
AI_SYSTEM_PERSONA = "Global Su Ayak İzi Uzmanı"

AI_OUTPUT_EXAMPLE = """{
  "product": "ürün adı",
  "water_footprint_liters": 1200,
  "risk_level": "Kritik",
  "production_zone": "Konum adı",
  "alternative_product": "daha iyi alternatif",
  "saved_water_liters": 500
}"""


def build_ai_prompt(product_query: str) -> str:
    """AI modeli için güvenli ve formatı kısıtlı prompt metni üretir."""
    return (
        f"Sistem Mesajı (Persona): {AI_SYSTEM_PERSONA}\n\n"
        "Görev:\n"
        f"Kullanıcının sorguladığı ürün için su ayak izi analizi yap: '{product_query}'.\n\n"
        "Çıktı Kısıtları:\n"
        "1) Kesinlikle SADECE saf ve geçerli bir JSON objesi döndür.\n"
        "2) Düz metin, markdown, kod bloğu (```json) veya açıklama ekleme.\n"
        "3) Sadece aşağıdaki anahtarları kullan:\n"
        '   "product", "water_footprint_liters", "risk_level", '
        '"production_zone", "alternative_product", "saved_water_liters"\n'
        "4) Numeric alanlar sayı olmalı.\n"
        '5) "risk_level" değeri yalnızca "İyi", "Orta" veya "Kritik" olmalı.\n\n'
        "Örnek format (birebir şema referansı):\n"
        f"{AI_OUTPUT_EXAMPLE}\n\n"
        "Cevabına hiçbir açıklama veya giriş cümlesi ekleme. "
        "Sadece JSON formatında veri döndür."
    )
